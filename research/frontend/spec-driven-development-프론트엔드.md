...
프론트엔드에서 spec-first가 실패하는 이유 중 상당 부분은 **스펙의 형식 문제**다.

- 텍스트 스펙이 아니라 **Figma + Design Token + Storybook** 조합이면 얘기가 달라진다.
- 이건 사실상 "시각적 언어로 된 스펙"이다.
- 실제로 많은 팀이 이 방식으로 spec-driven을 하고 있다.

### 프론트엔드도 레이어별로 다르다

"프론트엔드는 본질적으로 iterative하다"는 것은 **부분적으로만** 맞다:

| 레이어 | Spec-First 적합성 |
|--------|------------------|
| 시각/레이아웃 | iterative가 맞음 |
| 상태 관리 | spec-first가 충분히 가능 |
| 이벤트 흐름 | spec-first가 더 나음 |
| API 연동 | spec-first가 더 나음 |

같은 프론트엔드 코드베이스 안에서도 레이어마다 spec-first의 적합성이 다르다. 이걸 하나로 묶으면 "UI는 spec-first가 안 된다"는 **너무 강한 결론**이 되어버린다.

### 제안한 해결책은 이미 업계 표준

| 이 글의 제안 | 업계에서 부르는 것 |
|-------------|-------------------|
| 동작만 정의하는 스펙 | Acceptance Criteria (AC) |
| 레퍼런스 기반 시각 스펙 | Figma + Design System |
| spec → 초안 → 대화형 수정 | Agile / BDD의 실제 운용 방식 |

즉 "spec-first를 포기하자"가 아니라 **"프론트엔드에 맞는 스펙 형식을 쓰자"**가 더 정확한 결론이다.

### 한 줄 요약

> "spec-first는 프론트엔드에 안 맞다"가 아니라 **"텍스트 스펙은 프론트엔드에 안 맞다 — 시각적 스펙 + 행동 스펙으로 분리해서 써야 한다"**로 가는 게 더 정확하고 실용적인 방향이다.
